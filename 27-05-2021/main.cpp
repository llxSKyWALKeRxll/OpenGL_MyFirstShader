...
void create_shaders()
{
	...
	omniShadowShader.createFromFiles("Shaders/OmniShadowMapVertex.txt", "Shaders/OmniShadowMapGeometry.txt", "Shaders/OmniShadowMapFragment.txt");
}
...
void omniShadowMapPass(PointLight* light)
{
	...
	uniform_model = omniShadowShader.getModelLocation();
...
void renderPass(glm::mat4 projectionMatrix, glm::mat4 viewMatrix)
{
	...
	shader_list[0].setPointLights(point_lights, pointLightsCtr, 3, 0);
	shader_list[0].setSpotLights(spot_lights, spotLightsCtr, 3 + pointLightsCtr, pointLightsCtr);
	shader_list[0].setDirectionalLightTransform(&ambient_light.calculateLightTransform());

	ambient_light.getShadowMap()->read(GL_TEXTURE2);
...
int main()
{
	...
	skyboxFaces.push_back("Textures/Skyboxes/S1/majesty_rt.tga");
	skyboxFaces.push_back("Textures/Skyboxes/S1/majesty_lf.tga");
	skyboxFaces.push_back("Textures/Skyboxes/S1/majesty_up.tga");
	skyboxFaces.push_back("Textures/Skyboxes/S1/majesty_dn.tga");
	skyboxFaces.push_back("Textures/Skyboxes/S1/majesty_bk.tga");
	skyboxFaces.push_back("Textures/Skyboxes/S1/majesty_ft.tga");
	...
	while (!mainWindow.getShouldClose())
	{
		...
		if (mainWindow.getKeys()[GLFW_KEY_LEFT_ALT])
		{
			...
			mainWindow.getKeys()[GLFW_KEY_LEFT_ALT] = false;
		}

		directionalShadowMapPass(&ambient_light);

		for (size_t i = 0; i < pointLightsCtr; i++)
		{
			omniShadowMapPass(&point_lights[i]);
		}

		for (size_t i = 0; i < spotLightsCtr; i++)
		{
