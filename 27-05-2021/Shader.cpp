...
void Shader::createFromFiles(const char* vLocation, const char* gLocation, const char* fLocation)
{
	std::string vString = readFile(vLocation);
	std::string gString = readFile(gLocation);
	std::string fString = readFile(fLocation);
	const char* vCode = vString.c_str();
	const char* gCode = gString.c_str();
	const char* fCode = fString.c_str();
...
void Shader::compileShader(const char* vCode, const char* gCode, const char* fCode)
{
	shader_id = glCreateProgram();
	if (!shader_id)
	{
	...
	addShader(shader_id, vCode, GL_VERTEX_SHADER);
	addShader(shader_id, gCode, GL_GEOMETRY_SHADER);
	addShader(shader_id, fCode, GL_FRAGMENT_SHADER);
...
void Shader::compileProgram()
{
	...
	uniformOmniLightPos = glGetUniformLocation(shader_id, "lightPos");
	uniformFarPlane = glGetUniformLocation(shader_id, "farPlane");
	...
	for (size_t i = 0; i < 6; i++)
	{
		...
		uniformLightMatrices[i] = glGetUniformLocation(shader_id, locBuff);
	}
	...
	for (size_t i = 0; i < MAX_POINT_LIGHTS + MAX_SPOT_LIGHTS; i++)
	{
		...
		uniformOmniShadowMaps[i].shadowMap = glGetUniformLocation(shader_id, locBuff);

		snprintf(locBuff, sizeof(locBuff), "omniShadowMaps[%d].farPlane", i);
		uniformOmniShadowMaps[i].farPlane = glGetUniformLocation(shader_id, locBuff);
	}
}
...
void Shader::setPointLights(PointLight* pLight, unsigned int pCount, unsigned int textureUnit, unsigned int offset)
{
